...
> **Running for free?** This fork has been patched to run entirely without paid
> services. Query expansion, self-query filtering, reranking, dataset
> generation, and final answer generation now go through `OPENAI_BASE_URL`,
> which defaults to Groq's free OpenAI-compatible API (or point it at a local
> Ollama server for a fully offline setup). AWS SageMaker training/deployment
> is now optional (`USE_FREE_LLM_FOR_GENERATION=true` by default), so no AWS
> costs are required to run the full crawl -> RAG -> generate flow. See
> `.env.example` for the free-tier values to use. Fine-tuning your own model
> still requires either a local/Colab GPU or paid SageMaker if you want that
> extra step.
