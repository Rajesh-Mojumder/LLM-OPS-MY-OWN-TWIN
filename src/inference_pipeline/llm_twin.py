...
class LLMTwin:
    def __init__(self, mock: bool = False) -> None:
        ...
        # By default (settings.USE_FREE_LLM_FOR_GENERATION = True) we skip AWS
        # SageMaker entirely and call a free OpenAI-compatible model (Groq,
        # local Ollama, etc.) configured through OPENAI_API_KEY / OPENAI_BASE_URL
        # / OPENAI_MODEL_ID in your .env file. This avoids any AWS costs.
        #
        # If you have deployed your fine-tuned model to a real SageMaker
        # endpoint and want to use it instead, set
        # USE_FREE_LLM_FOR_GENERATION=false in your .env file.
        self._use_free_llm = settings.USE_FREE_LLM_FOR_GENERATION
        if self._use_free_llm:
            self._llm_endpoint = self.build_free_llm_client()
        else:
            self._llm_endpoint = self.build_sagemaker_predictor()

    def build_free_llm_client(self):
    ...
    def build_sagemaker_predictor(self):
        import sagemaker
    ...
    def call_llm_service(self, messages: list[dict[str, str]]) -> str:
        ...
        if self._use_free_llm:
            # Free path: any OpenAI-compatible provider (Groq, local Ollama, etc.)
            chat_completion = self._llm_endpoint.chat.completions.create(
            ...
            # Paid path: your fine-tuned model deployed on AWS SageMaker.
